 ...
 */
function answerLocally(question = "", pet = null) {
...
/* ---------------------------------------------------------------------
   The guardrail is a system prompt, not a post-hoc filter. index.js also
   applies the keyword table on top, and that check can only raise the
   urgency the model returned — never lower it.
   ------------------------------------------------------------------ */
/**
 * Builds the system prompt for whichever pet the question was asked
 * about. `pet` comes from the request body — see index.js — and is
 * never merged with any other pet's data. No pet on the request means
 * no pet context in the prompt, not a leftover one from a previous call:
 * this function is pure, so there is nothing to leak between requests.
 */
function buildSystemPrompt(pet) {
  const context = pet && pet.name
    ? describePet(pet)
    : "No specific pet profile was provided for this question — answer with general care guidance only.";

  return `You are PetCare AI, the assistant inside a pet-owner dashboard.

Rules you must follow:
- You give general pet-care information only.
- You NEVER state, imply, or guess a diagnosis.
- If the described symptoms could be serious - difficulty breathing, choking,
  seizures, collapse, bleeding, pale gums, suspected poisoning, a swollen or
  hard abdomen, repeated unproductive retching, or sudden severe pain - set
  urgency to "emergency" and tell the owner to seek veterinary care now.
- Use urgency "soon" when a vet should be contacted today or if a symptom
  persists; use "routine" for general care questions.
- Keep the answer under 90 words. Be calm, specific and practical.
- Always point the owner to a qualified veterinarian for diagnosis.
- Never recommend a human medication, a dose, or a home remedy that could
  be harmful. If asked for a dose, tell them to check with their vet.
- Only use the pet context below for THIS pet. Never mention another pet's
  name, breed or medical details — if no context is given, stay general.

Context you may use: ${context}

Respond with JSON only, matching the provided schema.`;
}

function describePet(pet) {
  const age = pet.ageYears ? `${pet.ageYears}-year-old ` : "";
  const species = pet.species || "pet";
  const breed = pet.breed ? `, a ${pet.breed}` : "";
  const bits = [`the pet is ${age}${species}${breed} named ${pet.name}.`];

  const allergy = pet.specialInstructions?.allergy;
  const medication = pet.specialInstructions?.medication;
  const notes = pet.specialInstructions?.notes;
  if (allergy)    bits.push(`Known allergy or sensitivity: ${allergy}.`);
  if (medication) bits.push(`Medication instructions on file: ${medication}.`);
  if (notes)      bits.push(`Other notes: ${notes}.`);

  return bits.join(" ");
}

module.exports = { answerLocally, DISCLAIMER, buildSystemPrompt };
