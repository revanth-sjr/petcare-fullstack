...
import { fmtClock, fmtLastDone, fmtDate, now, istTimeToday } from "./time.js";
...
function renderNext(dash, ctx) {
  ...
  $("#nextTime").textContent   = `${fmtClock(slotToday(row.slot))} · ${row.dosage}`;
...
function renderAlerts(dash, ctx) {
  ...
  const label = feeding ? "feeding" : row.name;

  strip.hidden = false;
  strip.className = `alert-strip${overdueRow ? "" : " is-due"}`;
  strip.innerHTML = `
    <span>${overdueRow ? "⚠️" : (feeding ? "🍖" : "💊")}</span>
